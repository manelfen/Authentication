...
func (s *SmartContract) RegisterDevice(ctx contractapi.TransactionContextInterface, id, cert, serverID, edgeID, regionID string) (string, error) {
	...
	device := Device{
		...
		Revoked:                     false,
		InterregionalAuthTimestamp:  "",
		InterregionalAuthCounter:    0,
...
func (s *SmartContract) AuthenticateDeviceInterregional(ctx contractapi.TransactionContextInterface, id, certPem, signatureB64, requestTimestamp, serverID, edgeID, regionID string) (string, error) {
	...
	if device.InterregionalAuthTimestamp != "" {
		if err := verifyTimestamp(device.InterregionalAuthTimestamp); err == nil {
			device.InterregionalAuthCounter++
			if device.InterregionalAuthCounter >= 3 {
				s.RevokeDevice(ctx, id)
				return createResponse(false, "too many interregional requests – device revoked")
			}
			_ = s.updateDevice(ctx, device)
			return createResponse(false, "interregional session still valid")
		}
		device.InterregionalAuthTimestamp = ""
		device.InterregionalAuthCounter = 0
		_ = s.updateDevice(ctx, device)
		return createResponse(false, "interregional session expired – please retry later")
	}

	if device.RegionalAuthTimestamp == "" {
		return createResponse(false, "no valid session – please authenticate regionally first")
	}
	if err := verifyTimestamp(device.RegionalAuthTimestamp); err != nil {
		return createResponse(false, "regional session expired – please authenticate regionally again")
	}
	...
	newTS := strconv.FormatInt(txTimestamp.Seconds*1000, 10) // millisecondes

	device.InterregionalAuthenticated = true
	device.InterregionalAuthTimestamp = newTS
	device.InterregionalAuthCounter = 0
...
func (s *SmartContract) SetRegionalAuthenticated(ctx contractapi.TransactionContextInterface, id string, value bool, timestamp string) error {
	device, err := s.GetDevice(ctx, id)
	if err != nil {
		return err
	}
	device.RegionalAuthenticated = value
	if value {
		device.RegionalAuthTimestamp = timestamp
	} else {
...
func (s *SmartContract) RevokeDevice(ctx contractapi.TransactionContextInterface, id string) {
	...
	device.InterregionalAuthenticated = false
	_ = s.updateDevice(ctx, device)
}
